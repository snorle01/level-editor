...
class square_base_model(base_model):
    def __init__(self, engine, position: tuple[int, int, int], scale: tuple[int, int, int], color: glm.vec3 = glm.vec3(1.0, 0.0, 0.0)):
...
class square_frame_model(square_base_model):
    def __init__(self, engine, position, scale, color = glm.vec3(1, 0, 0)):
        super().__init__(engine, position, scale, color)
